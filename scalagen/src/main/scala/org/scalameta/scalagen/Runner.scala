...
case class TransmutationResult(in: Stat, out: List[Stat])
...
case class Runner(generators: Set[Generator], recurse: Boolean = false) {
  ...
  def debug[A <: Tree](name: String, tree: A, pretty: Boolean = true): A = {
  ...
  private def applyGenerator[A <: Tree](in: A, g: Generator): A = {
    g match {
      ...
      case p: ParameterGenerator => applyParameterGenerator(in, p)
    }
  }

  private def applyParameterGenerator[A <: Tree](param: A, generator: ParameterGenerator): A = {
    val res = param match {
      case tp: Term.Param =>
